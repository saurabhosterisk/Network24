...
class SyncManager(private val context: Context) {
    ...
    suspend fun syncFullEpg(force: Boolean = false): SyncResult = withContext(Dispatchers.IO) {
        try {
            ...
            com.network24.player.core.cache.MemoryCache.clear()
...
// -------------------------
    ...
    private fun parseXmlTvTimeToMs(value: String?): Long {
        ...
        // Keep only first 14 chars: yyyyMMddHHmmss
        val main = value.take(14)
        return try {
            val sdf = SimpleDateFormat("yyyyMMddHHmmss", Locale.US)
            // NOTE: XMLTV time is usually local for provider; timezone offset is separate.
            // For simplicity we treat it as UTC if offset parsing not implemented.
            sdf.timeZone = TimeZone.getTimeZone("UTC")
            sdf.parse(main)?.time ?: 0L
        } catch (e: Exception) {
            0L
        }
    }
